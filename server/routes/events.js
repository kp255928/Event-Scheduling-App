...
router.route('/checkconflict').get((req,res)=>{
    ...
    if(event.length = 0) return res.status(400).json("No any events added")
    else{
        for(i=0; i<event.length; i++){
           if(sdate == event.sdate && edate == event.edate){
               ...
               return res.status(400).json("there is conflict on the same day")
           }
