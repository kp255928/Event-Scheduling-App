...
router.route('/checkconflict').get((req,res)=>{
    ...
    if(event.length = 0) return res.send("No any events added")
    else{
        for(i=0; i<event.length; i++){
           if(sdate == event.sdate && edate == event.edate){
               ...
               return res.send("there is conflict on the same day")
           }
